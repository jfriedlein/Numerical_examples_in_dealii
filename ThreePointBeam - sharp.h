...
// 3d grid:
	...
	void make_grid( Triangulation<3> &triangulation, const Parameter::GeneralParameters &parameter, std::vector<unsigned int> Vec_boundary_id_collection )
	{
		...
		Point<dim> pN_right( 2*notchWidth, 0,  0);	// the size of the notch body is twice a wide as the notch because only the innermost cells after the refinement will be deleted to create the notch
		Point<dim> pN_left( -2*notchWidth, height, thickness);
		
		std::vector<unsigned int> repetitions_notch (3);
		repetitions_notch[0]=8;
